...
class Noise(Signal):
    ...
    def __init__(self, amp=1.0, ys = None, SNR=1.0):
...
class UncorrelatedGaussianNoise(Noise):
    ...
    def evaluate(self, ts):
        ...
        #根据信噪比构造高斯噪声
        if(np.all(self.ys == 0) == False):
            ys_o = np.sum(abs(self.ys) ** 2) / len(self.ys)
            ys_n = ys_o / 10 ** (self.SNR / 10.0)
            ys_input = np.random.rand(len(self.ys)) * np.sqrt(ys_n)
        else:
            raise ValueError("原始信号不能为空")
